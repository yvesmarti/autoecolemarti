...
├── _redirects                    # 38 règles 301 explicites (canonicalise /<page>.html → /<page>) — généré manuellement
...
│
├── blog/                         # 22 articles + index
│   ├── index.html                # Blog hub with category filters
│   ├── 100-questions-permis-b-jour-examen.html
...
│   ├── permis-lyceen-etudiant-bayonne.html
...
│
├── plaquettes/                   # Brochures PDF — referenced in formules pages + espace élèves
│   ├── livret-100-questions-permis-b.pdf   # Livret des 100 questions (article blog + espace élèves)
│   ├── Plaquette_AAC.pdf
...
**Blog articles list (22 total):**
| File | Topic | Category |
|------|-------|----------|
| `100-questions-permis-b-jour-examen.html` | 100 questions de l'épreuve pratique + livret PDF | Outils pédagogiques |
...
- **Cloudflare Pages** — Hosting/deploy (connected to this GitHub repo; auto-deploys on push). Serves clean URLs natively and **auto-redirects** `/<page>.html` → `/<page>` and `/index.html` → `/` with a **308 (permanent)** redirect. En complément, un fichier `_redirects` (38 règles **301** explicites, généré manuellement) canonicalise également les URLs `.html`. Custom domain + `www` → apex + "Always Use HTTPS" are managed in the Cloudflare dashboard, not in the repo (no `CNAME` file). `404.html` at the repo root is served automatically as the not-found page.
