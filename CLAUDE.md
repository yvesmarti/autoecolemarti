...
/autoecolemarti/
├── index.html                    # Homepage
├── faq.html                      # FAQ page — 12 Q&A with JSON-LD FAQPage schema
...
### FAQ (`faq.html`)
- 12 Q&A items covering: hours required, manual vs. auto, pricing, AAC, documents, payment plans, exam format, probationary period, supervised driving, lesson locations
...
- **Cloudflare Pages** — Hosting/deploy (connected to this GitHub repo; auto-deploys on push). Serves clean URLs natively and **auto-redirects** `/<page>.html` → `/<page>` and `/index.html` → `/` with a **308 (permanent)** redirect. Custom domain + `www` → apex + "Always Use HTTPS" are managed in the Cloudflare dashboard, not in the repo (no `CNAME` file). `404.html` at the repo root is served automatically as the not-found page.
