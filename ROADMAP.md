...
### ~~Changement de grille tarifaire~~ — fait le 28/08/2026 (effet au 31/08/2026)
Bascule appliquée sur les 19 fichiers du site (procédure et détail dans `TARIFS.md`, §1
« Historique » et §3), et les 3 plaquettes PDF (`plaquettes/Plaquette_AAC.pdf`,
`Plaquette_Permis_B_Manuel.pdf`, `Plaquette_Permis_B_BVA.pdf`) régénérées par Yves le même
jour avec la nouvelle grille et la date d'effet. Rubrique close.

La procédure reste documentée dans `TARIFS.md` pour la prochaine bascule : il suffira de
demander à Claude *« Mets à jour les tarifs du site selon TARIFS.md — voici la nouvelle
grille : … »*.
